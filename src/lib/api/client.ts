...
export async function createDeviceClaimCode(data: {
    ...
    if (USE_MOCK) {
        await delay(300);
        const container = MOCK_CONTAINERS.find(
            (item) => item.container_id === data.container_id,
        );
        ...
        return {
            items: MOCK_DEVICES.map((device) => ({
                ...
                config: cloneDeviceConfig(
                    MOCK_DEVICE_CONFIGS.get(device.device_id) ??
                        device.config ??
...
export async function bootstrapDevice(
    ...
    if (USE_MOCK) {
        ...
        if (
            MOCK_DEVICES.some(
                (device) => device.factory_device_id === data.factory_device_id,
            )
        ) {
...
export async function updateDeviceConfig(
    ...
    if (USE_MOCK) {
        ...
        const container = MOCK_CONTAINERS.find(
            (item) => item.assigned_device_id === deviceId,
        );
        if (container) {
...
export async function getAlarms(
    ...
    if (USE_MOCK) {
        ...
        if (filters.container_id)
            items = items.filter(
                (a) => a.container_id === filters.container_id,
            );
        return { items, total: items.length };
    }
    const params = new URLSearchParams();
...
export async function resolveAlarm(
    ...
    if (USE_MOCK) {
        ...
        if (alarm) {
            ...
            alarm.ended_at = new Date().toISOString();
        }
        return;
    }
...
export async function getCrewPositions(): Promise<CleaningCrew[]> {
    ...
    type PositionItem = Omit<CleaningCrew, "current_location"> & {
...
export async function sendCrewLocation(
...
export async function dispatchRoutePlan(routePlanId: string): Promise<void> {
    if (USE_MOCK) {
        ...
        const plan = MOCK_ROUTE_PLANS.find(
            (p) => p.route_plan_id === routePlanId,
        );
        if (plan) {
            plan.status = "DISPATCHED";
            plan.dispatched_at = new Date().toISOString();
        }
        return;
    }
    await http(`/routes/plans/${routePlanId}/dispatch`, {
        method: "POST",
        body: JSON.stringify({}),
    });
}
    ...
    if (USE_MOCK) {
        ...
        for (const plan of MOCK_ROUTE_PLANS) {
            ...
            for (const route of plan.routes) {
                ...
                if (stop) {
                    (stop as unknown as Record<string, unknown>).status =
                        data.status;
...
export async function getMaintenanceTickets(
    ...
    if (USE_MOCK) {
        ...
        if (filters.container_id)
            items = items.filter(
                (t) => t.container_id === filters.container_id,
            );
        return items;
    }
    const params = new URLSearchParams(filters as Record<string, string>);
    const res = await http<{ items: MaintenanceTicket[] }>(
...
export async function updateMaintenanceTicket(
    ...
    if (USE_MOCK) {
        await delay(300);
        const ticket = MOCK_MAINTENANCE_TICKETS.find(
...
export function getMediaUrl(mediaId: string): string {
    return `${BASE_URL}/media/${mediaId}/file`;
}

export async function fetchMediaBlob(mediaId: string): Promise<string> {
    const token = getStoredToken();
    const res = await fetch(getMediaUrl(mediaId), {
